...
void menu(string name, void (*func)(int));

void FCFS(int batchsize)
{
	queue<int> q;
	int task_id = 1;
	now_time = 0;
...
void run(void (*func)(int), int batchsize){
	DataDeal();
	func(batchsize);
	PrintResults("tmp", batchsize);
	for(int i=1;i<=batchsize;i++){
		batch[i] = 0;
	}
}
